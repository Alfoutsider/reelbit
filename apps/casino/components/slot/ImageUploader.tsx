...
export function ImageUploader({ value, onChange }: Props) {
  ...
  const uploadFile = useCallback(async function uploadFile(file: File) {
    if (!ACCEPTED.includes(file.type)) { setError("Only JPG, PNG, GIF, or WEBP allowed."); return; }
    ...
    try {
      ...
      const r   = await fetch(`${API}/upload/slot-image`, {
        method:  "POST",
  ...
  return (
    <div className="space-y-2">
        ...
        {(["drop", "url"] as Mode[]).map((m) => (
          <button key={m} type="button" onClick={() => setMode(m)}
            className={cn(
              ...
              mode === m ? "bg-[#d4a017]/20 text-[#f5c842]" : "text-white/30 hover:text-white/50"
            )}>
            {m === "drop" ? "UPLOAD" : "URL"}
          </button>
        ))}
            ...
            <div className="relative rounded-xl overflow-hidden border border-white/10 bg-black/30">
              ...
              {uploading && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/50">
                  <div className="w-8 h-8 rounded-full border-2 border-[#d4a017]/30 border-t-[#d4a017] animate-spin" />
                </div>
              )}
            </div>
          ) : (
            <div
              ...
              className={cn(
                ...
                dragging
                  ...
                  : "border-white/10 bg-white/[0.02] hover:border-[#d4a017]/40 hover:bg-[#d4a017]/5"
              )}>
              {uploading ? (
                <div className="w-8 h-8 rounded-full border-2 border-[#d4a017]/30 border-t-[#d4a017] animate-spin" />
              ) : (
                <>
                  <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center transition-all",
                    dragging ? "bg-[#d4a017]/20" : "bg-white/5")}>
                    {dragging
                      ? <ImageIcon size={22} style={{ color: "#d4a017" }} />
      ...
      <p className="flex items-center gap-1.5 text-[11px] text-white/25 font-rajdhani">
        <ImageIcon size={10} style={{ color: "rgba(212,160,23,0.5)" }} />

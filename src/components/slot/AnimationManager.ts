...
export function drawAnticipationEffect(
  ...
  // Pulsing glow
  const glowAlpha = 0.2 + Math.sin(time * 0.2) * 0.15;
  const grad = ctx.createLinearGradient(x, y, x, y + h);
  grad.addColorStop(0, `rgba(0, 255, 136, ${glowAlpha * 0.3})`);
  grad.addColorStop(0.5, `rgba(0, 255, 136, ${glowAlpha})`);
  grad.addColorStop(1, `rgba(0, 255, 136, ${glowAlpha * 0.3})`);
  ctx.fillStyle = grad;
  roundRectPath(ctx, x, y, w, h, 8);
  ctx.fill();

  // Scanning line
  const scanY = y + ((time * 3) % h);
  const scanGrad = ctx.createLinearGradient(x, scanY - 10, x, scanY + 10);
  scanGrad.addColorStop(0, "rgba(0,255,136,0)");
  scanGrad.addColorStop(0.5, `rgba(0,255,136,${0.2 + Math.sin(time * 0.15) * 0.1})`);
  ...
  // Edge glow border
  ctx.strokeStyle = `rgba(0, 255, 136, ${0.4 + Math.sin(time * 0.25) * 0.2})`;
  ctx.lineWidth = 2;
...
/** Draw win symbol highlight with pulsing glow */
export function drawWinHighlight(
  ctx: CanvasRenderingContext2D,
  x: number, y: number, w: number, h: number,
  color: string, flashProgress: number
) {
  const pulse = Math.sin(flashProgress * Math.PI * 4) * 0.5 + 0.5;
  const alpha = pulse * 0.35;
  ...
  // Border glow
  ctx.strokeStyle = color + Math.floor((0.3 + pulse * 0.4) * 255).toString(16).padStart(2, "0");
  ctx.lineWidth = 2;
